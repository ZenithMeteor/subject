...
int main()
{
    ...
    /*
    //產生DataBase
    fp.open("Database.txt", ios::out);//開啟檔案
    ...
    */
    ...
    //讀取資料庫建成link list 
    fp.open("Database.txt", ios::in);//開啟檔案
    if(!fp){//如果開啟檔案失敗，fp為0；成功，fp為非0
        cout<<"Fail to open file: DB file"<<endl;
        system("PAUSE");
        return 0;
    ...
    fp1.open("Tag_file.txt", ios::in);//開啟檔案
    ...
    fp2.open("B_Time.txt", ios::out);//開啟檔案
    ...
    while(fp1>>t_n_EPC>>t_N1>>t_N2>>t_M1>>t_M2)
    {
        ...
        do
        {
            ...
            c_size=sizeof(now->n_EPC)+(sizeof(now->k_new)/4)+(sizeof(t_N1)/4)+(sizeof(t_N2)/4)-1;//kold new size一樣 
